...
df_input = pd.read_excel('data/idoc-knobloch.xlsx',
                         usecols=['sediment_depth_m', 'absolute_elevation_dhhn', 'depth_sat_zone', 'rel_elevation_point1', 'temp_c', 'meas_station', 'site_name'],
                         # skiprows=[1]
                         )
...
for site in df_input['site_name'].unique().tolist():
    ...
    ax.fill_between(x=[20, 32], y1=0, y2=-1, color='#1f77b4', alpha=0.3)
    ax.set_xlabel('T [°C]')
    ...
    ax.set_xticks(np.arange(20, 33, x_tick_interval))
    ...
    plt.tight_layout()
    fig.savefig('t-' + str(site) + '.png', dpi=300)
