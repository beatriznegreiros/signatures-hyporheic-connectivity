...
df_input = pd.read_csv('data/merged_kf_sr.csv')
df_input = df_input.dropna(subset=['depth_sat_zone'])
...
for site in df_input['site_name'].unique().tolist():
    ...
    for meas in meas_array_site_list:
        ...
        df_toplot.plot(x='slurp_rate_final',
                       y='depth_sat_zone',
    ...
    plt.tight_layout()
    fig.savefig('sr-' + str(site) + '.png', dpi=300)
